...
//            printf("key:%s\nlen=%d\n",linebuf+19,strlen(linebuf+19));
            SHA1((unsigned char *) &linebuf + 19, strlen(linebuf + 19),
...
int send_frame_length(int fd, int payload_length) {
    char *response_head;
    int head_length = 0;
...
jint Java_com_dftc_debug_serv_terminal_TermWebSocket_socket(JNIEnv *env, jobject thiz) {
    printf("Java_com_dftc_debug_serv_terminal_TermWebSocket_socket\n");
    ser_fd = socket(AF_INET, SOCK_STREAM, 0);
    return passive_server(ser_fd, 20);
}

jint Java_com_dftc_debug_serv_terminal_TermWebSocket_accept(JNIEnv *env, jobject thiz) {
    printf("Java_com_dftc_debug_serv_terminal_TermWebSocket_accept\n");
    struct sockaddr_in client_addr;
    socklen_t addr_length = sizeof(client_addr);
    conn = accept(ser_fd, (struct sockaddr *) &client_addr, &addr_length);
    printf("accept conn:%d\n", conn);
    shakehands(conn);
    return conn;
}

void Java_com_dftc_debug_serv_terminal_TermWebSocket_run(JNIEnv *env, jobject thiz) {

    printf("Java_com_dftc_debug_serv_terminal_TermWebSocket_run\n");
    while (1) {
        frame_head head;
        int rul = recv_frame_head(conn, &head);
        if (rul < 0) {
            printf("recv_frame_head rul < 0\n");
            break;
        }
        printf("fin=%d\nopcode=0x%X\nmask=%d\npayload_len=%llu\n", head.fin, head.opcode, head.mask,
               head.payload_length);
        ...
        do {
            ...
            jclass TestProvider = (*env)->FindClass(env,
                                                    "com/dftc/debug/serv/terminal/TermWebSocket");
            jmethodID onReceiveData = (*env)->GetStaticMethodID(env, TestProvider, "onReceiveData",
                                                                "([BI)Ljava/lang/String;");

            // 把BYTE数组转化为jbyteArray
            jbyteArray jarrRV = (*env)->NewByteArray(env, rul);//创建一个byte数组

            (*env)->SetByteArrayRegion(env, jarrRV, 0, rul, payload_data);

            (*env)->CallStaticObjectMethod(env, TestProvider, onReceiveData, jarrRV, rul);

            printf("size : %d\n", size);
            printf("head.payload_length : %llu\n", head.payload_length);
        } while (size < head.payload_length);
        printf("\n-----------\n");
...
Java_com_dftc_debug_serv_terminal_TermWebSocket_send(JNIEnv *env, jobject obj,
                                                                 jbyteArray jdata,
                                                                 jint jlength) {
    ...
    (*env)->GetByteArrayRegion(env, jdata, 0, len, jbarray);
    printf("jbarray: %d", jlength);

    char *payload_data = (char *) jbarray;
    printf("conn:%d\n", conn);
    send_frame_length(conn, jlength);
    int result = write(conn, payload_data, jlength);
    printf("result:%d\n", result);
    free(jbarray);
