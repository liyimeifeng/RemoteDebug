...
static int create_subprocess(JNIEnv *env, const char *cmd, char *const argv[], char *const envp[],
    ...
    errno = 0;
    int ptsResult = (int) ptsname_r(masterFd, devname, sizeof(devname));
    if (ptsResult && errno) {
...
Java_com_dftc_debug_serv_terminal_TermExec_sendSignal(JNIEnv *env, jobject clazz,
                                                                  jint procId, jint signal) {
    kill(procId, signal);
}

JNIEXPORT jint JNICALL
Java_com_dftc_debug_serv_terminal_TermExec_waitFor(JNIEnv *env, jclass clazz,
                                                               jint procId) {
    int status;
...
JNIEXPORT jint JNICALL
Java_com_dftc_debug_serv_terminal_TermExec_createSubprocessInternal(JNIEnv *env,
                                                                                jclass clazz,
                                                                                jstring cmd,
                                                                                jobjectArray args,
